...
export default function UploadId() {
  ...
  async function upload() {
    if (!file) return alert("Please upload your ID Proof");
      ...
      <div className="w-full max-w-xl">
            ...
            <div key={i} className="flex flex-col items-center">
              <div
                className={`w-4 h-4 rounded-full ${
                  i === 1 ? "bg-blue-600" : i < 1 ? "bg-green-500" : "bg-gray-300"
                }`}
              ></div>
              <span
                className={`text-xs mt-2 font-medium ${
                  i === 1 ? "text-blue-600" : i < 1 ? "text-green-600" : "text-gray-500"
                }`}
              >
                {step}
              </span>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 border border-gray-200">
          ...
          <div className="space-y-6">
            ...
            {file && (
              <div className="bg-blue-50 text-blue-700 p-3 rounded-xl text-sm font-medium border border-blue-200">
                Selected: {file.name}
              </div>
            )}
            ...
            >
              {loading ? "Uploading..." : "Next → Upload Address Proof"}
            </button>
          </div>
        </div>
      </div>
